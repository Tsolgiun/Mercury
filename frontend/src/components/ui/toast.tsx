...
const Toast: React.FC<ToastProps> = ({ toast, onDismiss }) => {
  ...
  const getBgColor = () => {
    switch (toast.type) {
      ...
      default:
        return 'bg-bg border-outline text-fg';
    }
  };
  ...
  return (
    <div
      className={`flex items-center p-4 mb-3 border rounded-md shadow-md ${getBgColor()} transform transition-all duration-300 ease-in-out`}
      role="alert"
    >
      ...
      <button
        type="button"
        className="ml-4 text-muted hover:text-fg"
